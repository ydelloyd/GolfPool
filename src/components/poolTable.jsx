...
function PoolTable(props) {
  ...
  useEffect(()=>{
    if(Object.keys(props.leaderboard).length > 0){
      // console.log(props.leaderboard["Phil Mickelson"])
      let temp = teams.map((e)=>{
        let toPar = [props.leaderboard[e.player1].toPar, props.leaderboard[e.player2].toPar, props.leaderboard[e.player3].toPar, props.leaderboard[e.player4].toPar, props.leaderboard[e.player5].toPar];
        ...
        <TableBody>
          {results.map((row) => (
            <TableRow key={row.team}>
              ...
              <TableCell>{row.player1 + ` (${props.leaderboard[row.player1].toPar})`}</TableCell>
              <TableCell>{row.player2 + ` (${props.leaderboard[row.player2].toPar})`}</TableCell>
              <TableCell>{row.player3 + ` (${props.leaderboard[row.player3].toPar})`}</TableCell>
              <TableCell>{row.player4 + ` (${props.leaderboard[row.player4].toPar})`}</TableCell>
              <TableCell>{row.player5 + ` (${props.leaderboard[row.player5].toPar})`}</TableCell>
